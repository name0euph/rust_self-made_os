#![no_std] // stdクレートを使わない
#![no_main] // mainのエントリポイントを利用しない UEFIのefi_main
#![feature(offset_of)]
...
type EfiVoid = u8;
type EfiHandle = u64;
type Result<T> = core::result::Result<T, &'static str>; // Tryトレイトを実装
...
const EFI_GRAPHICS_OUTPUT_GUID: EfiGuid = EfiGuid {
    data0: 0x9042a9de,
    data1: 0x23dc,
    data2: 0x4a38,
...
#[no_mangle]
fn efi_main(_image_handle: EfiHandle, efi_system_table: &EfiSytemTable) {
    // フレームバッファのアドレスを取得し、塗りつぶす
    ...
    let mut w = VramTextWriter::new(&mut vram); // インスタンス生成
    for i in 0..4 {
    ...
    for e in memory_map.iter() {
        if e.memory_size != EfiMemoryType::CONVENTIONAL_MEMORY {
            continue;
        }
...
// フレームバッファをマッピングしたインターフェース
// 所有権: 値の持ち主は1つ。
// 参照: 所有権を移さずに借りる。&T（不変参照）、&mut T（可変参照）。
// 借用規則: 可変参照は同時に1つだけ。不変参照は複数OK。可変と不変の同時は不可。
trait Bitmap {
    fn bytes_per_pixel(&self) -> i64; // &selfは自分自身を参照する
    fn pixels_per_line(&self) -> i64;
    fn width(&self) -> i64;
    fn height(&self) -> i64;
    fn buf_mut(&mut self) -> *mut u8;

    // フレームバッファ内の指定座標(x, y)のピクセル先頭アドレスを返す
...
fn init_vram(efi_system_table: &EfiSytemTable) -> Result<VramBufferInfo> {
...
fn draw_point<T: Bitmap>(
    ...
    *(buf.pixel_at_mut(x, y).ok_or("Out of Range")?) = color;  // 範囲チェック
    Ok(())
}
...
// ia/daの進行度に応じて、傾きdb/daの線のもう一方の軸の整数座標を求める関数
...
fn draw_line<T: Bitmap>(
    ...
    let sx = (x1 - x0).signum(); // 符号を返す(-1, 0, 1)
    let dy = (y1 - y0).abs();
    let sy = (y1 - y0).signum();
...
fn draw_font_fg<T: Bitmap>(buf: &mut T, x: i64, y: i64, color: u32, c: char) {
    ...
    if let Some(font) = lookup_font(c) {
        // font.iter()は各行への参照、 &[char; 8]を順に返す
        // .enumerate()で各要素の連番インデックス(0~15になる)をつけて、(index, item)のタプルとして返す
